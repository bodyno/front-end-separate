...
afterTran = function() {
  var changeCode, initRequire, refHandle, setref, tempEle, tempMessage, tempRule;
  initRequire = function(filed, txt) {
    tempRule[filed] = {};
    tempMessage[filed] = {};
    tempRule[filed]["required"] = true;
    return tempMessage[filed]["required"] = txt;
  };
  changeCode = function() {
  ...
  setref = function(refId) {
    $("#affiliateId").val(refId);
    return $("#affiliateId-hidden").val(refId);
  };
  tempRule = {
    custCode: {
  ...
  $("#birthDate").prop("readonly", "readonly").datepicker({
    ...
    onSelect: function(date, ele) {
      return $(ele.input).trigger("blur");
    }
  });
  $("#reg-btn").click(function() {
    return $(this).commitWithError($("#reg-form"), "/register", (function() {
      return location.href = "/register_success";
    }), function() {
      $(".reg-code-img").trigger("click").prev().val("");
      return $(".reg-code-img").parents(".reg-group").removeClass("ok");
    });
  });
  $("#reg-form").keyup(function(e) {
    if (e.keyCode === 13) {
      return $("#reg-btn").trigger("click");
    }
  });
